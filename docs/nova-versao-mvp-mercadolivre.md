...
**Achado de teste real (a confirmar oficialmente, não tratar como regra confirmada):** um link copiado pelo usuário enquanto navegava logado na própria conta de afiliado do Mercado Livre veio com parâmetros como `matt_tool_id`, `matt_event_ts` e `source=affiliate-profile` — alguns na query string, outros depois de `#` (fragmento da URL). Isso sugere que o método "oficial" de geração de link do Mercado Livre pode ser baseado em sessão (navegar autenticado como afiliado e copiar a URL resultante), não em uma fórmula simples de substituição de parâmetro replicável pelo sistema. Duas implicações técnicas já tratadas na implementação:
- O fragmento da URL (depois de `#`) nunca é enviado ao servidor numa requisição HTTP — qualquer etapa que resolva/"limpe" o link via requisição de rede tem que reanexar o fragmento original manualmente, ou perde silenciosamente dados de rastreio que estejam lá.
- Enquanto o mecanismo oficial não for confirmado na documentação/painel do Mercado Livre, o caminho mais seguro no MVP é o usuário colar manualmente o link gerado pela própria sessão de afiliado (campo `affiliate_link` editável na tela da oferta), em vez de o sistema tentar reconstruir esses parâmetros por conta própria.
...
9. Os parâmetros `matt_*` observados em links copiados da sessão de afiliado do usuário (ver seção 5) são de fato o mecanismo de rastreio de comissão do Mercado Livre, ou apenas metadados de navegação/recomendação sem relação com comissão? Só o painel oficial de relatórios de afiliado pode confirmar isso.
10. Esses parâmetros `matt_*` são específicos da conta/sessão que os gerou (e portanto não podem ser reaproveitados por outra conta/produto) ou existe uma forma de gerá-los programaticamente? Isso decide se "colar o link já gerado manualmente" precisa continuar sendo o método padrão por mais tempo.
